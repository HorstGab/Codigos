...
arqInv* buscarPalavraBTree(FILE *arqBTree, FILE *arqRegistro, char *palavra){

    BTreeHeader* headerBTree = readBTreeHeader(arqBTree);
    BTreeNode* raiz = readBTreeNode(arqBTree, headerBTree->root);
    BTreeNode* no = raiz;
    arqInv* foundArqInv = NULL;

    int posicao, found;

    if(raiz){
        ///enquanto n encontrar a palavra e n for folha
        while(!(found = searchBTreePos(no, palavra, &posicao)) && !isLeaf(no)){
            BTreeNode* proxNo = readBTreeNode(arqBTree, no->children[posicao]);
            free(no);
            no = proxNo;
        }

        ///se existe le o registro da posicao
        if(found) foundArqInv = readArquivoInvertido(arqRegistro, no->keys[posicao].regPos);
        free(no);
    }
    free(headerBTree);
    return foundArqInv;
...
void printBTree(FILE *f) {
    ...
    if ((f = fopen("arqInvertido.bin", "rb+")) == NULL) return;
...
void printBTreeNodeKeys(BTreeNode *node) {
    ...
    if (node->keyNum) {
        ...
        printf("%s", node->keys[node->keyNum - 1].id);
    } else {
        printf("!");
    }
    printf("]");
