...
def main():
    ...
    layout = [
        ...
        [
            sg.Text('Image Input', size=(60, 1), justification='center'),
            sg.Text('Image Output', size=(60, 1), justification='center')
        ],
        [
            sg.Image(filename='', key='-IMAGE INPUT-'),
            sg.Image(filename='', key='-IMAGE OUTPUT-')
        ],
    ...
    window = sg.Window('Filtros', layout)
    #img_in = cv2.imread()
    ...
    while True:
        ...
        window['-IMAGE INPUT-'].update(data=imgbytes)
        window['-IMAGE OUTPUT-'].update(data=imgbytes)
