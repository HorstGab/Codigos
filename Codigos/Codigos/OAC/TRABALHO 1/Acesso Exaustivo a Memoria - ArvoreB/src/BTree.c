...
void insertBTree(FILE *f, int id, int regPos) {
    ...
    RegData data = {data.id = id, data.regPos = regPos};
...
void removeBTree(FILE *fTree, int id) {
    ...
    RegData data = {data.id = id, data.regPos = -1};
...
void printBTreeNodeKeys(BTreeNode *node) {
    ...
    if (node->keyNum) {
        ...
        printf("%d", node->keys[node->keyNum - 1].id);
    } else {
        printf("!");
    }
    printf("]");
