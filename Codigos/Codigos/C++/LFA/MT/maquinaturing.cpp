...
void maquinaTuring::setEstadoFinal(string *nome)
{
    Estado *estado = this->adicionarEstado(nome);
...
{
    Estado *origem = this->adicionarEstado(nomeOrigem);
    Estado *destino = this->adicionarEstado(nomeDestino);

    if(this->contemSimbolo(simboloLido) && this->contemSimbolo(simboloEscrito) && movimentoValido(direcao)){
...
bool maquinaTuring::testeSequenciaDoEstado(string *sequencia, int posicao, Estado *estadoAtual)
{
    ...
    cout << "%s" << sequencia->substr(0, posicao) << "[%s]" << estadoAtual->getNome() << "%s"  << sequencia->substr(posicao) << endl;
